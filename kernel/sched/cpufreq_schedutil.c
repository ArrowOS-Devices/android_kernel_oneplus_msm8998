...
static void sugov_set_rate_limits(struct sugov_policy *sg_policy, bool state)
{
	if (sg_policy->limit_state != state) {
		...
		swap(sg_policy->up_rate_delay_ns, 
				...
				unsigned int flags)
{
	...
	if (flags & SUGOV_LIMIT) {
		...
		sugov_set_rate_limits(sg_policy, (flags & SUGOV_LIMIT_SWAP));
...
static int sugov_start(struct cpufreq_policy *policy)
{
	...
	sg_policy->limit_state = false;
	sg_policy->up_rate_delay_ns = 500000ULL;
	sg_policy->down_rate_delay_ns = 2000000ULL;
